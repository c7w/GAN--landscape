    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            for i, (imgs, _) in enumerate(self.dataloader):
                ...
                #! latent layer 取为 64
                z = jt.array((np.random.normal(0, 1, (imgs.shape[0], 64))).astype('float32'))
                fake_imgs = self.generator(z)
                real_validity = self.discriminator(real_imgs)
                fake_validity = self.discriminator(fake_imgs)
                gradient_penalty = compute_gradient_penalty(self.discriminator, real_imgs, fake_imgs)
                d_loss = (- real_validity.mean() + fake_validity.mean() + self.lambda_gp * gradient_penalty)
                d_loss.sync()
                ...
                if ((i % self.critic) == 0):
                    ...
                    fake_validityg = self.discriminator(fake_img)
                    g_loss = -fake_validityg.mean()
                    g_loss.sync()
                    self.optimizer_G.step(g_loss)
                    
                    if self.warmup_times==-1:
                        print(('[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]' % (epoch, self.max_epochs, i, len(self.dataloader), d_loss.data, g_loss.data)))
                        if ((self.batches_done % self.sample_interval) == 0):
                            save_image(fake_imgs.data[:25], ('images/%d.png' % self.batches_done), nrow=5)
                        self.batches_done += self.critic

                if self.warmup_times!=-1:
                    jt.sync_all()
                    self.cnt += 1
                    if self.cnt == self.warmup_times:
                        jt.sync_all(True)
                        sta = time.time()
                    if self.cnt > self.warmup_times + self.run_times:
                        jt.sync_all(True)
                        total_time = time.time() - sta
                        print(f"run {self.run_times} iters cost {total_time} seconds, and avg {total_time / self.run_times} one iter.")
                        exit(0)
...
#! TODO::
    ...
    def _parse_args(self, args: Namespace):
        ...
        # GP
        self.lambda_gp = config['GP']['lambda_gp']
        self.critic = config['GP']['critic']
        self.warmup_times = config['GP']['warmup_times']
        self.batches_done = config['GP']['batches_done']
        self.sample_interval = config['GP']['sample_interval']
        self.cnt = config['GP']['cnt']
        self.run_times = config['GP']['run_times']
