...
class WGAN_Discriminator(nn.Module):
    ...
    def __init__(self, in_channels=3):
        ...
        def discriminator_block(in_filters, out_filters, stride=2, normalization=True):
            ...
            if normalization:
                layers.append(nn.InstanceNorm2d(out_filters, eps=1e-05, momentum=0.1, affine=True))
            layers.append(nn.LeakyReLU(scale=0.2))
            return layers

        in_channels = 6  ## TODO: Try different in_channels?

        self.d1 = nn.Sequential(discriminator_block(in_channels, 64, normalization=False))  # (B, 64, 192, 256)
        ...
        self.weight = nn.Sequential(
            nn.Linear(in_features=4, out_features=1),  # No Tanh...
        )
