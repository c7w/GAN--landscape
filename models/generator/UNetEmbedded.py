...
class UnetGeneratorEmbedded(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        super(UnetGeneratorEmbedded, self).__init__()

        self.embedding = nn.Embedding(29, 32)
        input_nc = 32
    ...
    def execute(self, input):
        input = ((input + 1) / 2 * 255).round().long()[:, 0, :, :]
        input = self.embedding(input).transpose(0, 3, 1, 2)
